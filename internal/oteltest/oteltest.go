...
// If opts are provided and are braintrust.Options, it will enable Braintrust tracing.
// Otherwise, it returns a plain OTel tracer.
func Setup(t *testing.T, opts ...braintrust.Option) (oteltrace.Tracer, *Exporter) {
	...
	// Only enable Braintrust tracing if opts are provided
	if len(opts) > 0 {
